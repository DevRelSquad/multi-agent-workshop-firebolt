---
title: "TODO: Multi-Agent Integration"
...
3. **Orchestrating workflows** - Coordinating multi-step operations (analytics → report → email)
...
User Query → parseIntent() → routeTask() → handleMultiStepQuery()
              ↓               ↓              ↓
           Gemini AI     Agent Type    Analytics → Report → Email`} />
...
Look for the `TODO: Exercise 1` comment in the file. Implement the `parseIntent` method.
...
- "Show me revenue" → `{intent: "analytics", entities: {query_type: "revenue"}}`
- "Generate report and email to john@example.com" → `{intent: "multi_step", entities: {recipient: "john@example.com"}}`
- "Top selling products" → `{intent: "analytics", entities: {query_type: "top_products"}}`
...
Look for the `TODO: Exercise 2` comment in the file. Implement the `routeTask` method in `src/lib/agents/orchestrator.ts`.
...
**Routing rules:**
- `intent: 'analytics'` → `'analytics'` agent
- `intent: 'report'` → `'report'` agent
...
Look for the `TODO: Exercise 3` comment in the file. The entire orchestration workflow is already implemented - you just need to complete **3 lines** at the top!
...
**✨ Everything else is done for you!** The code already:
- ✅ Executes analytics queries
- ✅ Generates financial/summary reports
- ✅ Sends emails with proper subjects
- ✅ Tracks steps and handles errors
...
Look for the `TODO: Exercise 4` comment in the file. Open `src/app/api/orchestrator/route.ts` and implement both POST and GET endpoints.
...
// → { total_revenue: 1234567, total_purchases: 8542, ... }
...
// → "Executive Summary: Total revenue of $1.23M from 8,542 purchases..."
...
// → Email preview logged to console (sandbox mode)
...
**Intent Parsing:**
- ✅ "Show me revenue" → analytics intent, revenue query_type
- ✅ "Top selling products" → analytics intent, top_products query_type
- ✅ "Generate report" → report intent
- ✅ "Email to user@test.com" → email intent with recipient

**Multi-Step Execution:**
- ✅ "Show revenue and send report to alice@example.com" → 3 steps
- ✅ "Top products and email to manager@company.com" → 3 steps
- ✅ "Generate financial report for brand analysis" → 2 steps
- ✅ "Analyze user behavior" → 1 step

**Error Handling:**
- ✅ Invalid GEMINI_API_KEY returns 500 error
- ✅ Missing query parameter returns 400 error
- ✅ Gemini parsing failure throws descriptive error

**Email Preview:**
- ✅ Open browser console (F12)
- ✅ Check for "📧 Email Preview (Sandbox Mode)" output
- ✅ Verify recipient, subject, and body are logged
