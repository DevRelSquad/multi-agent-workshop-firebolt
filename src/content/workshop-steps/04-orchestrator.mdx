...
2. **Multi-step orchestration** - Chain analytics → report → email workflows
...
```
User Query → Parse Intent → Route to Agent(s) → Execute → Return Results
```
...
## Exercise 2: Complete the Analytics Agent executeQuery (Still TODO)
...
## Exercise 3: Complete the Report Agent generateReport (TODO)

**📝 File:** `src/lib/agents/report.ts`
...
## Exercise 4: Implement Multi-Step Query Orchestration (TODO)
...
<Checkpoint items={[
  "parseIntent method works correctly (already implemented ✅)",
  ...
  "Multi-step queries chain analytics → report → email successfully",
...
┌─────────────────┐
│ Step 1:         │
│ Analytics Query │  ← AnalyticsAgent
│ (revenue)       │
└────────┬────────┘
         │
         ▼
...
┌─────────────────┐
│ Step 3:         │
│ Send Email      │  ← ReportAgent.sendEmail()
│ (to recipient)  │
└─────────────────┘
```
...
**Test scenarios:**
1. ✅ **Simple analytics**: "Show me revenue" → Should execute analytics query only
2. ✅ **Report generation**: "Generate a revenue report" → Should run analytics + report
3. ✅ **Email delivery**: "Show top products and email to test@example.com" → Should run analytics + report + email
4. ✅ **Natural language**: "What are my best selling products?" → Should parse intent and execute
...
### 4. Add Task Routing (Already Implemented ✅)
...
- ✅ Parses user intent with Gemini AI (already implemented)
- ✅ Coordinates multiple agents (Analytics, Report)
- ✅ Executes multi-step workflows (analytics → report → email)
